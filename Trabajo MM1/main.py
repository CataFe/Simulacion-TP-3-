...
def simular_inventario(
    ...
    for dia in range(1, horizonte + 1):
        llegadas_hoy = [p for p in pedidos_pendientes if p["dia_llegada"] == dia]
        pedidos_pendientes = [p for p in pedidos_pendientes if p["dia_llegada"] > dia]
        for pedido in llegadas_hoy:
            stock += pedido["cantidad"]

        demanda = rng.poisson(demanda_media)
        ventas = min(stock, demanda)
        faltante = demanda - ventas
        stock -= ventas

        costo_mant_dia = stock * costo_mantenimiento
        ...
        if inventario_en_posicion <= s:
            pedidos_pendientes.append({"dia_llegada": dia + lead_time, "cantidad": Q})
            costo_orden_dia = costo_fijo_orden
    ...
    for indice_politica, (politica, valores) in enumerate(parametros["politicas"].items()):
        for corrida in range(1, parametros["corridas"] + 1):
            ...
            resultado = simular_inventario(
                ...
                lead_time=parametros["lead_time"],
            ...
            if corrida == 1:
                ...
                historial["Q"] = valores["Q"]
                historiales.append(historial)

    df_corridas = pd.DataFrame(corridas)
    df_promedios = (
        df_corridas.groupby(["politica", "s", "Q"], as_index=False)
        .agg(
...
def generar_graficos_inventario(df_promedios, df_historiales):
    ...
    ax.set_title("Inventario en el tiempo")
    ax.set_xlabel("Dia")
    ax.set_ylabel("Stock")
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def imprimir_resumen(df_mm1_promedios, df_denegacion, df_inv_promedios):
    print("\nRESUMEN FINAL")
    print("=" * 60)
    ...
    print(
        ...
        f"(s={int(mejor['s'])}, Q={int(mejor['Q'])}) con costo total {mejor['costo_total']:.2f}"
    )
...
def main():
    ...
    imprimir_resumen(df_mm1_promedios, df_mm1_denegacion, df_inv_promedios)
